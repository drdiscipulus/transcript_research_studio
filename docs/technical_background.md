...
Local language-model support is optional and is separate from transcription and speaker detection. Transcription uses faster-whisper, optional speaker detection uses pyannote, and the AI-assisted Codes and Transcript Analysis workflows use a separately installed Ollama or LM Studio instance.

The sidecar probes the providers through loopback-only HTTP connections:

- Ollama: `http://127.0.0.1:11434`
- LM Studio: `http://127.0.0.1:1234`

The provider process and its local API server must already be running, and at least one model must be available through that provider. Transcript Research Studio discovers the provider's local model catalog and sends prompts to the model explicitly selected by the user. It does not install providers, download or manage general-purpose language models, start externally installed provider services, or fall back to a cloud AI service.

In Codes, the provider is used only for advisory evidence, code, note, codebook, and theme suggestions. In Transcript Analysis, it is used for the selected built-in or custom transcript analysis. Provider requests are restricted to loopback addresses; no LAN or internet-facing server configuration is required.
